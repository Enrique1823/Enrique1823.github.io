...
if (isset($_GET['txtID'])) {
    ...
    // Obtener el nombre de la imagen del integrante que se va a eliminar
    ...
    // Preparar y ejecutar la consulta para eliminar el integrante con el ID especificado
    $sentencia = $conexion->prepare("DELETE FROM tbl_equipo WHERE id=:id ");
    $sentencia->bindParam(":id", $txtID);
    $sentencia->execute();

    // Redireccionar a 'index.php' con un mensaje de éxito
    $mensaje = "Integrante eliminado con éxito. ";
    header("Location:index.php?mensaje=" . $mensaje);
}

// Preparar y ejecutar la consulta para obtener la lista de integrantes del equipo
$sentencia = $conexion->prepare("SELECT * FROM `tbl_equipo`");
$sentencia->execute();
$lista_tbl_equipo = $sentencia->fetchAll(PDO::FETCH_ASSOC);
        ...
        <table class="table">
            <thead>
                <tr>
                    ...
                    <th scope="col">Redes_Sociales</th>
                    <th scope="col">Acciones</th>
                </tr>
            </thead>
            <tbody>
            <?php foreach($lista_tbl_equipo as $registros){?>
                <tr class="">
